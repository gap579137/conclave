...
This guide explains how to run Conclave reliably in Matrix rooms, including end-to-end encrypted (E2EE) rooms.
...
3. `room_id` is correct:
   ...
   - supported: room alias (`#alias:server`) and Conclave will resolve it
...
user_id = "@conclave:matrix.example.com"
...
- Conclave attempts to read identity from Matrix `/_matrix/client/v3/account/whoami`.
...
3. Confirm Conclave logs contain Matrix listener startup and no repeated sync/auth errors.
