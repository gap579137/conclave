# Conclave Security Improvement Roadmap
...
Conclave already has **excellent application-layer security**:
...
## Comparison: Conclave vs PicoClaw vs Production Grade

| Feature | PicoClaw | Conclave Now | Conclave + Roadmap | Production Target |
|---------|----------|--------------|-------------------|-------------------|
...
| Security self-test (`conclave audit --check`) | 1 day | Low |
...
[security.audit]
enabled = true
log_path = "~/.config/conclave/audit.log"
sign_events = true
max_size_mb = 100
...
conclave security --check
# → ✓ Sandbox: Firejail active
# → ✓ Audit logging enabled (42 events today)
# → → Resource limits: 512MB mem, 50% CPU

# Audit log queries
conclave audit --user @alice --since 24h
conclave audit --risk high --violations-only
conclave audit --verify-signatures

# Sandbox test
conclave sandbox --test
# → Testing isolation...
...
**Conclave is already more secure than PicoClaw** with:
...
**By implementing this roadmap**, Conclave becomes:
...
**Value**: Transforms Conclave from "safe for testing" to "safe for production"
