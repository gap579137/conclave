...
This guide explains how to use the `conclave-tools` Python package for consistent tool calling with any OpenAI-compatible LLM provider.

## Background

Some LLM providers, particularly Chinese models like GLM-5 (Zhipu AI), have inconsistent tool calling behavior when using text-based tool invocation. Conclave's Rust core uses structured tool calling via the OpenAI API format, but some models respond better to a different approach.
...
```python
import asyncio
from conclave_tools import create_agent, shell, file_read, file_write
...
```python
from conclave_tools import tool, create_agent
...
```python
import os
from conclave_tools.integrations import DiscordBot
...
# Single message
conclave-tools "What is the current date?"

# Interactive mode
conclave-tools -i
```

## Comparison with Rust Conclave

| Aspect | Rust Conclave | conclave-tools |
|--------|---------------|-----------------|
