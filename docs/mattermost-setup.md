...
Conclave supports native integration with Mattermost via its REST API v4. This integration is ideal for self-hosted, private, or air-gapped environments where sovereign communication is a requirement.
...
2.  **Bot Account**:
    ...
    - Set a username (e.g., `conclave-bot`).
...
| `mention_only` | (Optional) When `true`, only messages that explicitly mention the bot username (for example `@conclave-bot`) are processed. Default: `false`. |

## Threaded Conversations

Conclave supports Mattermost threads in both modes:
- If a user sends a message in an existing thread, Conclave always replies within that same thread.
...
When `mention_only = true`, Conclave applies an extra filter after `allowed_users` authorization:
