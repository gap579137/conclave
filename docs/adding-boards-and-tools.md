# Adding Boards and Tools — Conclave Hardware Guide

This guide explains how to add new hardware boards and custom tools to Conclave.

## Quick Start: Add a Board via CLI

```bash
# Add a board (updates ~/.conclave/config.toml)
conclave peripheral add nucleo-f401re /dev/ttyACM0
conclave peripheral add arduino-uno /dev/cu.usbmodem12345
conclave peripheral add rpi-gpio native   # for Raspberry Pi GPIO (Linux)
...
With the `rag-pdf` feature, Conclave can index PDF files:
...
2. **Add to config** — `conclave peripheral add my-board /dev/ttyUSB0`
...
| Command | Description |
|---------|-------------|
| `conclave peripheral list` | List configured boards |
| `conclave peripheral add <board> <path>` | Add board (writes config) |
| `conclave peripheral flash` | Flash Arduino firmware |
| `conclave peripheral flash-nucleo` | Flash Nucleo firmware |
| `conclave hardware discover` | List USB devices |
| `conclave hardware info` | Chip info via probe-rs |
