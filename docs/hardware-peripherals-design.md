# Hardware Peripherals Design — Conclave

Conclave enables microcontrollers (MCUs) and Single Board Computers (SBCs) to **dynamically interpret natural language commands**, generate hardware-specific code, and execute peripheral interactions in real-time.

## 1. Vision

**Goal:** Conclave acts as a hardware-aware AI agent that:
...
**Mental model:** Conclave = brain that understands hardware. Peripherals = arms and legs it controls.
...
Conclave runs **directly on the device**. The board spins up a gRPC/nanoRPC server and communicates with peripherals locally.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│  Conclave on ESP32 / Raspberry Pi (Edge-Native)                             │
│                                                                             │
│  ┌─────────────┐    ┌──────────────┐    ┌─────────────────────────────────┐ │
...
2. Conclave fetches board-specific docs (e.g. ESP32 GPIO mapping)
...
Conclave runs on the **host** and maintains a hardware-aware link to the target. Used for development, introspection, and flashing.

```
┌─────────────────────┐                    ┌──────────────────────────────────┐
│  Conclave on Mac    │   USB / J-Link /   │  STM32 Nucleo-F401RE              │
...
2. Conclave identifies connected hardware (VID/PID, architecture)
...
2. Conclave writes/flashes via OpenOCD or probe-rs
3. Confirms success

**Or:**
1. Conclave auto-discovers: *"STM32 Nucleo on /dev/ttyACM0, ARM Cortex-M4"*
...
|------------------|--------------------------------|----------------------------------|
| Conclave runs on | Device (ESP32, RPi)           | Host (Mac, Linux)                |
...
Host runs Conclave; peripheral runs minimal firmware. Simple JSON over serial.

### Mode B: RPi as Host (Native GPIO)

Conclave on Pi; GPIO via rppal or sysfs. No separate firmware.
...
conclave agent --mode edge

# Host-Mediated: connect to USB/J-Link target
conclave agent --peripheral nucleo-f401re:/dev/ttyACM0
conclave agent --probe jlink

# Hardware introspection
conclave hardware discover
conclave hardware introspect /dev/ttyACM0
```
...
board = "esp32"
transport = "wifi"
# Edge-Native: Conclave runs on ESP32
```
...
1. **Startup:** Conclave loads config, sees `peripherals.boards`.
...
For low-latency, typed RPC between Conclave and peripherals:
...
- **conclave-firmware** or **conclave-peripheral** — a separate crate/workspace.
...
- User flashes this to the board; Conclave connects and discovers capabilities.

## 9. Implementation Phases

### Phase 1: Skeleton ✅ (Done)

- [x] Add `Peripheral` trait, config schema, CLI (`conclave peripheral list/add`)
- [x] Add `--peripheral` flag to agent
- [x] Document in AGENTS.md

### Phase 2: Host-Mediated — Hardware Discovery ✅ (Done)

- [x] `conclave hardware discover`: enumerate USB devices (VID/PID)
- [x] Board registry: map VID/PID → architecture, name (e.g. Nucleo-F401RE)
- [x] `conclave hardware introspect <path>`: memory map, peripheral list
...
- [x] Conclave on Raspberry Pi (native GPIO via rppal)
...
- [x] `conclave-esp32` firmware crate (`firmware/conclave-esp32`) — GPIO over UART
- [x] ESP32 in hardware registry (CH340 VID/PID)
- [ ] Conclave *on* ESP32 (WiFi + LLM, edge-native) — future
- [ ] Wasm or template-based execution for LLM-generated logic

**Usage:** Flash `firmware/conclave-esp32` to ESP32, add `board = "esp32"`, `transport = "serial"`, `path = "/dev/ttyUSB0"` to config.
...
- Running full Conclave *on* bare STM32 (no WiFi, limited RAM) — use Host-Mediated instead
...
> *"Boards like ESP, Raspberry Pi, or boards with WiFi can connect to an LLM (Gemini or open-source). Conclave runs on the device, creates its own gRPC, spins it up, and communicates with peripherals. User asks via WhatsApp: 'move X arm' or 'turn on LED'. Conclave gets accurate documentation, writes code, executes it, stores it optimally, runs it, and turns on the LED — all on the development board.*
>
> *For STM Nucleo connected via USB/J-Link/Aardvark to my Mac: Conclave from my Mac accesses the hardware, installs or writes what it wants on the device, and returns the result. Example: 'Hey Conclave, what are the available/readable addresses on this USB device?' It can figure out what's connected where and suggest."*
