...
[INFO] ✓ Audit logging enabled (~/.config/conclave/audit.log)
...
```bash
# Check what's active
$ conclave security --status
Security Status:
  ✓ Sandbox: Landlock (Linux kernel 6.2)
  ✓ Memory monitoring: 512MB limit
  ✓ Audit logging: ~/.config/conclave/audit.log
  → 47 events logged today

# Disable sandbox explicitly (writes to config)
$ conclave config set security.sandbox.enabled false

# Enable specific backend
$ conclave config set security.sandbox.backend firejail

# Adjust limits
$ conclave config set security.resources.max_memory_mb 2048
```
...
```bash
$ conclave onboard --security-level paranoid
[1/9] Workspace Setup...
...
...
✅ **Status visibility** — `conclave security --status`
