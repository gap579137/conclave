#!/bin/bash
# Conclave Telegram Integration Test Suite
...
if [ -f "target/release/conclave" ]; then
    BINARY_SIZE=$(ls -lh target/release/conclave | awk '{print $5}')
    SIZE_BYTES=$(stat -f%z target/release/conclave 2>/dev/null || stat -c%s target/release/conclave)
...
CONFIG_PATH="$HOME/.conclave/config.toml"
if [ -f "$CONFIG_PATH" ]; then
    ...
    if grep -q "\[channels_config.telegram\]" "$CONFIG_PATH"; then
        ...
        warn "Telegram not configured - run 'conclave onboard' first"
    fi
else
    warn "No config file found - run 'conclave onboard' first"
fi
...
START_TIME=$(date +%s)
HEALTH_OUTPUT=$(timeout 10 target/release/conclave channel doctor 2>&1 || true)
END_TIME=$(date +%s)
