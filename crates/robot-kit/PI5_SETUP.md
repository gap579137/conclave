...
Complete guide to setting up a Conclave-powered robot on Raspberry Pi 5.
...
mkdir -p ~/.conclave/models
cp models/ggml-base.bin ~/.conclave/models/
```
...
mkdir -p ~/.conclave/models/piper
cd ~/.conclave/models/piper
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx
wget https://huggingface.co/rhasspy/piper-voices/resolve/main/en/en_US/lessac/medium/en_US-lessac-medium.onnx.json

# Test
echo "Hello, I am your robot!" | piper --model ~/.conclave/models/piper/en_US-lessac-medium.onnx --output_file test.wav
aplay test.wav
```
...
### 7. Build Conclave Robot Kit

```bash
# Clone repo (or copy from USB)
git clone https://github.com/theonlyhennygod/conclave
cd conclave

# Build robot kit
cargo build --release -p conclave-robot-kit

# Build main conclave (optional, if using as agent)
cargo build --release
```
...
```bash
mkdir -p ~/.conclave
nano ~/.conclave/robot.toml
```

```toml
# ~/.conclave/robot.toml - Real Hardware Configuration
...
# Test speaker
echo "Testing speaker" | piper --model ~/.conclave/models/piper/en_US-lessac-medium.onnx --output_file - | aplay -D plughw:0,0
...
FIFO_PATH = "/tmp/conclave_sensors.fifo"
...
### Start Conclave Agent

```bash
# Configure Conclave to use robot tools
cat > ~/.conclave/config.toml << 'EOF'
...
cp ~/conclave/crates/robot-kit/SOUL.md ~/.conclave/workspace/

# Start agent
./target/release/conclave agent
```
...
# Start sensor loop
if [ ! -p /tmp/conclave_sensors.fifo ]; then
    mkfifo /tmp/conclave_sensors.fifo
fi
...
# Start conclave
cd ~/conclave
./target/release/conclave daemon &
AGENT_PID=$!
...
```bash
# /etc/systemd/system/conclave-robot.service
sudo tee /etc/systemd/system/conclave-robot.service << 'EOF'
[Unit]
Description=Conclave Robot
After=network.target ollama.service

[Service]
Type=simple
User=pi
WorkingDirectory=/home/pi/conclave
...
sudo systemctl enable conclave-robot
sudo systemctl start conclave-robot

# Check status
sudo systemctl status conclave-robot
journalctl -u conclave-robot -f  # View logs
```
