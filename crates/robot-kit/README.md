...
A complete toolkit for building AI-powered robots with Conclave. Designed for Raspberry Pi deployment with offline Ollama inference.
...
### 1. Build Conclave with robot tools
...
# Copy config
mkdir -p ~/.conclave
cp crates/robot-kit/robot.toml ~/.conclave/
cp crates/robot-kit/SOUL.md ~/.conclave/workspace/
...
# Test in mock mode
./target/release/conclave agent -m "Say hello and show a happy face"

# Test with real hardware
# (after configuring robot.toml)
./target/release/conclave agent -m "Move forward 1 meter"
```
...
```rust
use conclave_robot_kit::{create_tools, RobotConfig};
...
If you want runtime registration in `conclave`, add a thin adapter that maps this
...
mkdir conclave-robot-kit
cp -r target/release/conclave conclave-robot-kit/
cp -r examples/robot_kit conclave-robot-kit/
cp -r ~/.conclave conclave-robot-kit/dot-conclave

# Include models
mkdir -p conclave-robot-kit/models
cp ~/.conclave/models/ggml-base.bin conclave-robot-kit/models/
# Note: Ollama models are large, may want to download on target

# Create tarball
tar -czvf conclave-robot-kit.tar.gz conclave-robot-kit/

# Copy to USB
cp conclave-robot-kit.tar.gz /media/usb/TarBalls/
```
