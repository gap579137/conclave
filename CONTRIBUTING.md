...
Thanks for your interest in contributing to Conclave! This guide will help you get started.
...
Conclave supports layered secret management for local development and CI hygiene.
...
2. **Config file** (`~/.conclave/config.toml`)
    - Persistent setup for long-term use
    - When `secrets.encrypt = true` (default), secret values are encrypted before save
    - Secret key is stored at `~/.conclave/.secret_key` with restricted permissions
    - Use `conclave onboard` for guided setup
...
3. Generic env vars (`CONCLAVE_API_KEY`, `API_KEY`)

Provider/model config overrides:

- `CONCLAVE_PROVIDER` / `PROVIDER`
- `CONCLAVE_MODEL`
...
- `~/.conclave/.secret_key` or similar key files
...
- If identity-like wording is required, use Conclave-centric labels only (for example: `ConclaveAgent`, `ConclaveOperator`, `conclave_user`).
...
- **Identity normalization**: when identity traits are unavoidable, use Conclave/project-native roles instead of personal or real-world identities.
...
Conclave's architecture is built on **traits** — every subsystem is swappable. This means contributing a new integration is as simple as implementing a trait and registering it in the factory function.
...
- **Identity-like labels**: if unavoidable, use Conclave-native identifiers only (`ConclaveAgent`, `conclave_user`, `conclave_node`).
...
- **Good identity-like label**: `ConclaveAgent`, `conclave_user`, `conclave_node`
...
- [ ] Any required identity-like wording uses Conclave/project-native labels only
