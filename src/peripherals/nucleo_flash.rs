//! Flash Conclave Nucleo-F401RE firmware via probe-rs.
//!
...
/// Flash Conclave Nucleo firmware. Builds from firmware/conclave-nucleo.
pub fn flash_nucleo_firmware() -> Result<()> {
    ...
    // CARGO_MANIFEST_DIR = repo root (conclave's Cargo.toml)
    let repo_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"));
    let firmware_dir = repo_root.join("firmware").join("conclave-nucleo");
    if !firmware_dir.join("Cargo.toml").exists() {
        anyhow::bail!(
            "Nucleo firmware not found at {}. Run from conclave repo root.",
            firmware_dir.display()
        );
    }

    println!("Building Conclave Nucleo firmware...");
    ...
    println!("Conclave Nucleo firmware flashed successfully.");
