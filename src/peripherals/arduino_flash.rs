//! Flash Conclave Arduino firmware via arduino-cli.
//!
...
/// Conclave Arduino Uno base firmware (capabilities, gpio_read, gpio_write).
const FIRMWARE_INO: &str = include_str!("../../firmware/conclave-arduino/conclave-arduino.ino");

const FQBN: &str = "arduino:avr:uno";
const SKETCH_NAME: &str = "conclave-arduino";
...
/// Flash Conclave firmware to Arduino at the given port.
pub fn flash_arduino_firmware(port: &str) -> Result<()> {
    ensure_arduino_cli()?;
    ensure_avr_core()?;

    let temp_dir = std::env::temp_dir().join(format!("conclave_flash_{}", uuid::Uuid::new_v4()));
    ...
    // Compile
    println!("Compiling Conclave Arduino firmware...");
    ...
    println!("Conclave firmware flashed successfully.");
