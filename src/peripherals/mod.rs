...
/// Handle `conclave peripheral` subcommands.
    ...
    match cmd {
        crate::PeripheralCommands::List => {
            ...
            if boards.is_empty() {
                ...
                println!();
                println!("Add one with: conclave peripheral add <board> <path>");
                println!("  Example: conclave peripheral add nucleo-f401re /dev/ttyACM0");
                println!();
