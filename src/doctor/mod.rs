...
pub fn run(config: &Config) -> Result<()> {
    ...
    // Print report
    println!("🩺 Conclave Doctor (enhanced)");
    println!();
    ...
    if errors > 0 {
        println!("  💡 Fix the errors above, then run `conclave doctor` again.");
    }
...
pub fn run_models(config: &Config, provider_override: Option<&str>, use_cache: bool) -> Result<()> {
    ...
    println!("🩺 Conclave Doctor — Model Catalog Probe");
...
fn check_config_semantics(config: &Config, items: &mut Vec<DiagItem>) {
    ...
    if has_channel {
        ...
        items.push(DiagItem::warn(
            cat,
            "no channels configured — run `conclave onboard` to set one up",
        ));
    }
...
mod tests {
    ...
    fn workspace_probe_path_is_hidden_and_unique() {
        ...
        assert!(first
            ...
            .is_some_and(|name| name.starts_with(".conclave_doctor_probe_")));
    }
