...
mod tests {
    ...
    async fn file_read_existing_file() {
        let dir = std::env::temp_dir().join("conclave_test_file_read");
    ...
    async fn file_read_nonexistent_file() {
        let dir = std::env::temp_dir().join("conclave_test_file_read_missing");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_blocks_path_traversal() {
        let dir = std::env::temp_dir().join("conclave_test_file_read_traversal");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_blocks_when_rate_limited() {
        let dir = std::env::temp_dir().join("conclave_test_file_read_rate_limited");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
        tokio::fs::write(dir.join("test.txt"), "hello world")
            .await
    ...
    async fn file_read_allows_readonly_mode() {
        let dir = std::env::temp_dir().join("conclave_test_file_read_readonly");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_empty_file() {
        let dir = std::env::temp_dir().join("conclave_test_file_read_empty");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_nested_path() {
        let dir = std::env::temp_dir().join("conclave_test_file_read_nested");
        let _ = tokio::fs::remove_dir_all(&dir).await;
    ...
    async fn file_read_blocks_symlink_escape() {
        ...
        let root = std::env::temp_dir().join("conclave_test_file_read_symlink_escape");
    ...
    async fn file_read_nonexistent_consumes_rate_limit_budget() {
        let dir = std::env::temp_dir().join("conclave_test_file_read_probe");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
    ...
    async fn file_read_rejects_oversized_file() {
        let dir = std::env::temp_dir().join("conclave_test_file_read_large");
        let _ = tokio::fs::remove_dir_all(&dir).await;
        tokio::fs::create_dir_all(&dir).await.unwrap();
