...
fn build_models_help_response(current: &ChannelRouteSelection, workspace_dir: &Path) -> String {
    ...
    if cached_models.is_empty() {
        let _ = writeln!(
            response,
            "\nNo cached model list found for `{}`. Ask the operator to run `conclave models refresh --provider {}`.",
...
pub fn build_system_prompt(
    ...
    if prompt.is_empty() {
        "You are Conclave, a fast and efficient AI assistant built in Rust. Be helpful, concise, and direct.".to_string()
    } else {
        prompt
    }
}
...
fn bind_telegram_identity(config: &Config, identity: &str) -> Result<()> {
    ...
    let Some(telegram) = updated.channels_config.telegram.as_mut() else {
        anyhow::bail!(
            "Telegram channel is not configured. Run `conclave onboard --channels-only` first"
        );
    };
    ...
    match maybe_restart_managed_daemon_service() {
        ...
        Ok(false) => {
            println!(
                "ℹ️ No managed daemon service detected. If `conclave daemon`/`channel start` is already running, restart it to load the updated allowlist."
            );
        }
        Err(e) => {
            eprintln!(
                "⚠️ Allowlist saved, but failed to reload daemon service automatically: {e}\n\
                 Restart service manually with `conclave service stop && conclave service start`."
            );
        }
    }
    Ok(())
...
fn maybe_restart_managed_daemon_service() -> Result<bool> {
    if cfg!(target_os = "macos") {
        ...
        let plist = home
            .join("Library")
            .join("LaunchAgents")
            .join("com.conclave.daemon.plist");
        if !plist.exists() {
            return Ok(false);
        }
        ...
        if !listed.contains("com.conclave.daemon") {
            return Ok(false);
        }

        let _ = Command::new("launchctl")
            .args(["stop", "com.conclave.daemon"])
            .output();
        let start_output = Command::new("launchctl")
            .args(["start", "com.conclave.daemon"])
            .output()
    ...
    if cfg!(target_os = "linux") {
        ...
        let active_output = Command::new("systemctl")
            .args(["--user", "is-active", "conclave.service"])
            .output()
        ...
        let restart_output = Command::new("systemctl")
            .args(["--user", "restart", "conclave.service"])
            .output()
...
pub fn handle_command(command: crate::ChannelCommands, config: &Config) -> Result<()> {
    match command {
        ...
        crate::ChannelCommands::List => {
            ...
            println!("\nTo start channels: conclave channel start");
            println!("To check health:    conclave channel doctor");
            println!("To configure:      conclave onboard");
            Ok(())
        }
        crate::ChannelCommands::Add {
            ...
            anyhow::bail!(
                "Channel type '{channel_type}' — use `conclave onboard` to configure channels"
            );
        }
        crate::ChannelCommands::Remove { name } => {
            anyhow::bail!("Remove channel '{name}' — edit ~/.conclave/config.toml directly");
        }
...
pub async fn doctor_channels(config: Config) -> Result<()> {
    ...
    if channels.is_empty() {
        println!("No real-time channels configured. Run `conclave onboard` first.");
        return Ok(());
    }

    println!("🩺 Conclave Channel Doctor");
    println!();
    ...
    if config.channels_config.webhook.is_some() {
        println!("  ℹ️  Webhook   check via `conclave gateway` then GET /health");
    }
...
pub async fn start_channels(config: Config) -> Result<()> {
    ...
    let provider_runtime_options = providers::ProviderRuntimeOptions {
        ...
        conclave_dir: config.config_path.parent().map(std::path::PathBuf::from),
    ...
    if channels.is_empty() {
        println!("No channels configured. Run `conclave onboard` to set up channels.");
        return Ok(());
    }

    println!("🦀 Conclave Channel Server");
    println!("  🤖 Model:    {model}");
...
{"name":"mock_price","arguments":{"symbol":"BTC"}}
    ...
    fn channel_log_truncation_is_utf8_safe_for_multibyte_text() {
        let msg = "Hello from Conclave 🌍. Current status is healthy, and café-style UTF-8 text stays safe in logs.";
