...
// in `~/.conclave/.secret_key` with restrictive file permissions (0600). The
...
pub struct SecretStore {
    /// Path to the key file (`~/.conclave/.secret_key`)
    key_path: PathBuf,
...
impl SecretStore {
    ...
    pub fn new(conclave_dir: &Path, enabled: bool) -> Self {
        Self {
            key_path: conclave_dir.join(".secret_key"),
            enabled,
        }
    }
