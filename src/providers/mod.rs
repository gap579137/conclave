...
pub struct ProviderRuntimeOptions {
    ...
    pub conclave_dir: Option<PathBuf>,
...
/// 3. Generic fallback variables (`CONCLAVE_API_KEY`, `API_KEY`)
///
...
fn resolve_provider_credential(name: &str, credential_override: Option<&str>) -> Option<String> {
    ...
    for env_var in ["CONCLAVE_API_KEY", "API_KEY"] {
        if let Ok(value) = std::env::var(env_var) {
            let value = value.trim();
            if !value.is_empty() {
                return Some(value.to_string());
...
pub fn create_provider_with_url(
    ...
    match name {
        ...
        _ => anyhow::bail!(
            "Unknown provider: {name}. Check README for supported providers or run `conclave onboard --interactive` to reconfigure.\n\
...
/// Return the list of all known providers for display in `conclave providers list`.
///
