...
const DEFAULT_CODEX_INSTRUCTIONS: &str =
    "You are Conclave, a concise and helpful coding assistant.";
...
impl OpenAiCodexProvider {
    pub fn new(options: &ProviderRuntimeOptions) -> Self {
        let state_dir = options
            .conclave_dir
            .clone()
            .unwrap_or_else(default_conclave_dir);
...
fn default_conclave_dir() -> PathBuf {
    directories::UserDirs::new().map_or_else(
        || PathBuf::from(".conclave"),
        |dirs| dirs.home_dir().join(".conclave"),
    )
}
...
fn resolve_reasoning_effort(model_id: &str) -> String {
    let raw = std::env::var("CONCLAVE_CODEX_REASONING_EFFORT")
        .ok()
...
impl Provider for OpenAiCodexProvider {
    async fn chat_with_system(
        ...
        let access_token = self
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex auth profile not found. Run `conclave auth login --provider openai-codex`."
                )
            })?;
        let account_id = profile
            ...
            .ok_or_else(|| {
                anyhow::anyhow!(
                    "OpenAI Codex account id not found in auth profile/token. Run `conclave auth login --provider openai-codex` again."
                )
            })?;
