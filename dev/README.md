# Conclave Development Environment

A fully containerized development sandbox for Conclave agents. This environment allows you to develop, test, and debug the agent in isolation without modifying your host system.
...
### 2. Enter Agent Container (`conclave-dev`)

```bash
./dev/cli.sh agent
```

Use this to run `conclave` CLI commands manually, debug the binary, or check logs internally.
...
- **Agent**: `/conclave-data/workspace`
- **Sandbox**: `/home/developer/workspace`

Files created by the agent are visible to the sandbox user, and vice versa.

The agent configuration lives in `target/.conclave` (mounted to `/conclave-data/.conclave`), so settings persist across container rebuilds.
...
**Note:** This removes `target/.conclave` (config/DB) but leaves the `playground/` directory intact. To fully wipe everything, manually delete `playground/`.
...
- Rust compilation, tests, and audit/deny tools run in `conclave-local-ci` container.
...
- The root `Dockerfile` also caches Rust `target/` (`id=conclave-target`) to speed repeat local image builds.
