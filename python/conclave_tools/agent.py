...
SYSTEM_PROMPT = """You are Conclave, an AI assistant with tool access. Use tools to accomplish tasks.
...
class ConclaveAgent:
    ...
    def invoke(self, input: dict[str, Any], config: Optional[dict] = None) -> dict:
        ...
        raise RuntimeError(
            "ConclaveAgent.invoke() cannot be called inside an active event loop. "
            "Use 'await ConclaveAgent.ainvoke(...)' instead."
        )
...
def create_agent(
    ...
    """
    Create a Conclave agent with LangGraph-based tool calling.
    ...
    Returns:
        Configured ConclaveAgent instance
