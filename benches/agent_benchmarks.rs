//! Performance benchmarks for Conclave hot paths.
//!
//! Benchmarks cover:
...
use conclave::agent::dispatcher::{NativeToolDispatcher, ToolDispatcher, XmlToolDispatcher};
use conclave::config::MemoryConfig;
use conclave::memory;
use conclave::memory::{Memory, MemoryCategory};
...
<tool_call>
{"name": "search", "arguments": {"query": "conclave architecture"}}
</tool_call>
    ...
    let response = ChatResponse {
        ...
        tool_calls: vec![
            ToolCall {
                id: "tc1".into(),
                name: "search".into(),
                arguments: r#"{"query": "conclave"}"#.into(),
            },
            ToolCall {
                id: "tc2".into(),
                name: "read_file".into(),
...
fn bench_memory_operations(c: &mut Criterion) {
    ...
    rt.block_on(async {
        for i in 0..100 {
            mem.store(
                &format!("key_{i}"),
                &format!("Content entry number {i} about conclave agent runtime"),
                MemoryCategory::Core,
                None,
            )
            .await
    ...
    c.bench_function("memory_recall_top10", |b| {
        b.iter(|| {
            rt.block_on(async {
                mem.recall(black_box("conclave agent"), 10, None)
                    .await
                    .unwrap()
            })
        });
