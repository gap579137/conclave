# AGENTS.md — Conclave Agent Engineering Protocol
...
Conclave is a Rust-first autonomous agent runtime optimized for:
...
- If identity-like naming is required in tests/examples, use Conclave-native labels only (`ConclaveAgent`, `conclave_user`, `conclave_node`).
...
- If identity-like context is unavoidable, use Conclave-scoped roles/labels only (for example: `ConclaveAgent`, `ConclaveOperator`, `conclave_user`) and avoid real-world personas.
- Recommended identity-safe naming palette (use when identity-like context is required):
    - actor labels: `ConclaveAgent`, `ConclaveOperator`, `ConclaveMaintainer`, `conclave_user`
    - service/runtime labels: `conclave_bot`, `conclave_service`, `conclave_runtime`, `conclave_node`
    - environment labels: `conclave_project`, `conclave_workspace`, `conclave_channel`
